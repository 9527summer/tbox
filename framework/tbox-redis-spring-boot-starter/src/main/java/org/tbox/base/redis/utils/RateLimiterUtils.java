...
public class RateLimiterUtils {
     ...
     * @return 可用令牌数
     */
    public static long tokenBucketAvailable(String key) {
        RedissonClient client = getRedissonClient();
        if (client == null) {
            return -1;
        }
     ...
     * @param key 限流标识
     */
    public static void tokenBucketReset(String key) {
        RedissonClient client = getRedissonClient();
        if (client == null) {
            return;
        }
     ...
     * @param unit     时间单位
     */
    public static void tokenBucketSetRate(String key, long rate, long interval, TimeUnit unit) {
        RedissonClient client = getRedissonClient();
        if (client == null) {
            return;
        }
        ...
        String redisKey = TOKEN_BUCKET_PREFIX + key;
        RRateLimiter rateLimiter = client.getRateLimiter(redisKey);
        rateLimiter.setRate(RateType.OVERALL, rate, interval, convertToRateIntervalUnit(unit));
    }
